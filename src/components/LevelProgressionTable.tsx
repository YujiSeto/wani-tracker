...
function getSpeedInfo(days: number, isPassed: boolean, isCurrent: boolean) {
  if (!isPassed && !isCurrent) return { color: 'transparent', label: 'Locked', bg: 'bg-white/5' };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-wrap gap-6 items-end">
        ...
        <div className="flex-1 min-w-[200px] min-h-[80px] flex items-center p-4 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-sm">
          {activeLevelData ? (
            ...
            <div className="w-full">
              <p className="text-xs font-bold text-white/20 uppercase tracking-tighter">Level {displayLevel}</p>
              <p className="text-sm text-white/30 italic">({t(locale, 'levels.statusLocked')})</p>
            </div>
          )}
        </div>
      </div>
            ...
            return (
              <div
                ...
                className={`
                  ...
                  ${isCurrent ? 'animate-pulse ring-2 ring-white/50 z-20' : ''}
                  ${isHovered ? 'scale-110 z-30 ring-2 ring-white shadow-xl brightness-125' : 'hover:scale-105'}
                `}
      ...
      {/* ── Legend ──────────────────────────────────────────────── */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-y-4 pt-6 border-t border-white/5">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-4 flex-1">
          ...
          <div className="flex items-center gap-2 sm:gap-3 flex-1">
            <span className="text-[10px] font-bold uppercase text-rose-400 shrink-0">
              {t(locale, 'levels.fast')}
            </span>
            <div className="flex h-1.5 flex-1 max-w-[400px] rounded-full overflow-hidden bg-white/5">
              <div 
            ...
            <span className="text-[10px] font-bold uppercase text-indigo-400 shrink-0">
              {t(locale, 'levels.slow')}
            </span>
          </div>
        </div>

        <div className="flex items-center gap-3 px-3 py-1.5 rounded-lg bg-white/5 border border-white/5 animate-pulse w-fit sm:ml-4">
          <div className="w-2.5 h-2.5 rounded-full bg-white/20 ring-2 ring-white/50" />
